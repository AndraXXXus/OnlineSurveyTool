<div id="radio" class="row row-cols-xl-6 d-flex justify-content-center align-items-center">
    @forelse ($choices as $choice)
    <div class="col-xl-6">
        <div class="form-control m-2 @error('choice_id') is-invalid @enderror">
            ...
            <label title="{{ $choice->choice_text }}" for={{"choice_id_".$choice->id}} class="form-control-label">
                ...
                value="{{ $choice->id }}"
                onmousedown="this.tag = this.checked;"
                onclick="this.checked = !this.tag; handleRadioOnClick()"
                @checked( old('choice_id') ? $choice->id === old('choice_id') : (count($previous_answers_choice_ids)>0 ? in_array($choice->id, $previous_answers_choice_ids) : false) )
                />
                    {{ $choice->choice_text }}
            </label>
        </div>
...
<script>
    function handleRadioOnClick() {
        if (@js($question->question_required)==true) {
            ...
            if(submit_button){
                const any_input_checked = document.querySelectorAll('#radio input:checked').length > 0;
